...
function resetViews(){
  ...
  secSum.classList.add('hidden');
  secRisk.classList.add('hidden');
  secEnt.classList.add('hidden');
  pre.textContent = '';
  listSum.innerHTML = ''; riskLevel.textContent=''; riskLevel.dataset.level='';
...
$('ping').addEventListener('click', async (e)=>{
  e.preventDefault();
  try{
    const r = await chrome.runtime.sendMessage({type:'PING_BG'});
    pre.textContent = 'PING_BG: ' + (r?.ok?'OK':'ERR');
    secRes.classList.remove('hidden');
  }catch(err){ pre.textContent = 'Ping error: ' + err.message; secRes.classList.remove('hidden');}
});

btnAnalyze.addEventListener('click', async ()=>{
  const selected = task.value; // "translate" | "rewrite" | "proofread"
  if (selected === 'translate' || selected === 'rewrite' || selected === 'proofread') {
    return runSimple(selected); // usa o texto da caixa; não depende do content script
  }
  // fallback: analyze
  return runAnalyze();
});
...
async function runAnalyze(){
  ...
  try{
    ...
    if(!tab || !okUrl(tab.url)){ pre.textContent='Open an HTTP/HTTPS page and reload it.'; secRes.classList.remove('hidden'); return; }
    ...
    pre.textContent = 'Analyze error: ' + err.message;
    secRes.classList.remove('hidden');
  }
}
...
async function runSimple(kind){
  ...
  try{
    ...
    if(!text) throw new Error('Provide text in the box');
    const payload = { text }; 
    ...
    if(!ai?.ok) throw new Error(ai?.error || 'AI error');
    pre.textContent = ai.raw; 
    secRes.classList.remove('hidden');
  }catch(err){
    pre.textContent = 'Task error: ' + err.message; 
    secRes.classList.remove('hidden');
  }
}
